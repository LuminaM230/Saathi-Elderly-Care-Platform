...
// ---- Backend API client ----
// Talks to the Express server in /server. Falls back to localStorage
// automatically if the backend is unreachable (e.g. not started yet),
// so the app still works standalone in the prototype/demo environment.
...
async function apiGet(path, fallback) {
  try {
    ...
    console.warn(`API unreachable, using local fallback for ${path}:`, err.message);
    return fallback;
  }
}
...
async function apiSend(method, path, body) {
  try {
    ...
    console.warn(`API unreachable, change not persisted server-side for ${path}:`, err.message);
    return null;
  }
}

const api = {
  getPreferences: (fallback) => apiGet("/preferences", fallback),
  updatePreferences: (body) => apiSend("PUT", "/preferences", body),
  getContacts: (fallback) => apiGet("/contacts", fallback),
  addContact: (body) => apiSend("POST", "/contacts", body),
  updateContact: (id, body) => apiSend("PUT", `/contacts/${id}`, body),
  deleteContact: async (id) => {
    try {
      const res = await fetch(`${API_BASE}/contacts/${id}`, { method: "DELETE" });
      return res.ok;
    } catch (err) {
      console.warn("API unreachable, delete not persisted server-side:", err.message);
      return false;
    }
  },
};
...
// ---- Voice command recognition (real speech-to-text, not just TTS) ----
// Uses the browser's SpeechRecognition API to listen for a short phrase,
// then matches it against a small command table to navigate the app.
// Falls back gracefully (onUnsupported) on browsers without support.
const VOICE_COMMANDS = [
  { keywords: ["emergency", "help", "sos"], screen: "emergency", confirm: "Opening emergency screen." },
  { keywords: ["home", "dashboard"], screen: "home", confirm: "Going home." },
  { keywords: ["medicine", "medication", "scan medicine", "scan"], screen: "scanner", confirm: "Opening medicine scanner." },
  { keywords: ["reminder", "reminders"], screen: "reminders", confirm: "Opening reminders." },
  { keywords: ["report", "reports", "health report"], screen: "reports", confirm: "Opening health reports." },
  { keywords: ["check in", "checkin", "check-in"], screen: "checkin", confirm: "Opening check-in." },
  { keywords: ["family"], screen: "family", confirm: "Opening family screen." },
  { keywords: ["safety", "home safety"], screen: "safetyreport", confirm: "Opening home safety report." },
  { keywords: ["profile", "settings"], screen: "profile", confirm: "Opening your profile." },
];

function matchVoiceCommand(transcript) {
  const text = transcript.toLowerCase().trim();
  for (const cmd of VOICE_COMMANDS) {
    if (cmd.keywords.some((k) => text.includes(k))) return cmd;
  }
  return null;
}

const useVoiceCommands = (onNavigate) => {
  ...
  const start = () => {
    ...
    if (!SpeechRecognition) {
      setSupported(false);
      safeSpeak("Sorry, voice commands are not supported on this device.");
      return;
    }
    ...
    recognition.onresult = (event) => {
      ...
      setTranscript(heard);
      const match = matchVoiceCommand(heard);
      if (match) {
        safeSpeak(match.confirm);
        onNavigate(match.screen);
      } else {
        safeSpeak("Sorry, I didn't understand that command.");
      }
    };
  ...
  return { listening, start, stop, transcript, supported };
};

const VoiceCommandBtn = ({ onNavigate, size = 38, style = {} }) => {
  const { listening, start, stop, supported } = useVoiceCommands(onNavigate);
  return (
    <button
      onClick={() => (listening ? stop() : start())}
      title={supported ? (listening ? "Listening… tap to stop" : "Tap and say a command (e.g. \"emergency\", \"reminders\")") : "Voice commands not supported"}
      style={{
        width: size, height: size, borderRadius: "50%",
        background: listening ? theme.danger : "rgba(255,255,255,0.2)",
        border: `1.5px solid ${listening ? theme.danger : "rgba(255,255,255,0.3)"}`,
        display: "flex", alignItems: "center", justifyContent: "center",
        cursor: "pointer", flexShrink: 0, transition: "all 0.2s",
        boxShadow: listening ? `0 0 0 4px ${theme.danger}33` : "none",
        ...style,
      }}
    >
      <svg width={size * 0.44} height={size * 0.44} viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2.2" strokeLinecap="round">
        <path d="M12 1a4 4 0 0 1 4 4v6a4 4 0 0 1-8 0V5a4 4 0 0 1 4-4z" />
        <path d="M19 10a7 7 0 0 1-14 0" />
        {listening && <circle cx="12" cy="12" r="11" opacity="0.3" />}
      </svg>
    </button>
  );
};
...
const HomeScreen = ({ onNavigate, lang }) => {
  ...
  useEffect(() => {
    ...
    (async () => {
      const localFallback = (() => {
        try { return { watchConnected: localStorage.getItem("saathi_watch") === "1" }; } catch { return { watchConnected: false }; }
      })();
      const prefs = await api.getPreferences(localFallback);
      if (!cancelled && typeof prefs.watchConnected === "boolean") setWatchConnected(prefs.watchConnected);
    })();
  ...
  const toggleWatch = async () => {
    ...
    await api.updatePreferences({ watchConnected: next });
  };
      ...
      <div style={{ background: `linear-gradient(135deg, ${theme.primary}, ${theme.primaryDark})`, padding: "18px 18px 22px", borderRadius: "0 0 28px 28px", flexShrink: 0 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          ...
          <div style={{ display: "flex", gap: 8, alignItems: "center", flexShrink: 0 }}>
            ...
            <VoiceCommandBtn onNavigate={onNavigate} size={38} />
...
const EmergencyScreen = ({ onNavigate }) => {
  ...
  // On mount, try to load the source-of-truth list from the backend.
  // If the API is unreachable, we keep whatever localStorage/defaults gave us.
  useEffect(() => {
    let cancelled = false;
    (async () => {
      const remote = await api.getContacts(null);
      if (!cancelled && remote) {
        setContacts(remote);
        try { localStorage.setItem("saathi_emergency_contacts", JSON.stringify(remote)); } catch {}
      }
    })();
    return () => { cancelled = true; };
  }, []);
  ...
  const saveContact = async () => {
    ...
    if (editId) {
      ...
      const optimistic = { id: Date.now(), ...form, color: theme.primary };
      persistContacts([...contacts, optimistic]);
      const saved = await api.addContact(form);
      // Reconcile the temporary client-side id with the server-assigned id, if the API responded.
      if (saved && saved.id) {
        persistContacts([...contacts, optimistic].map(c => c.id === optimistic.id ? { ...optimistic, id: saved.id } : c));
      }
    }
              ...
              <Card key={c.id} style={{ padding: "12px 14px", border: `1.5px solid ${c.color}22` }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10, overflow: "hidden" }}>
                  ...
                  <div style={{ display: "flex", gap: 5, flexShrink: 0 }}>
                    ...
                    <button onClick={() => { persistContacts(contacts.filter(x => x.id !== c.id)); api.deleteContact(c.id); }}
